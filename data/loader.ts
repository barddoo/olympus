...
import mongo, {Db, InsertOneWriteOpResult, MongoCallback, WithId} from 'mongodb';
...
(async () => {
  ...
  await servidores(
    datasets.servidores.downloadUrl,
    datasets.servidores.fileName,
    destination
  );
  ...
  for (let [file, func] of fileDataset) {

    const csvData: Converter = csv({delimiter: ';'})
      .fromFile(path.join(destination, file), {encoding: 'latin1'});
...
async function loadEmployeeData(csvData: Converter, collection: Collection) {
  ...
  return new Promise<void>((resolve, reject) => {
    csvData.subscribe(async (data, index) => {
      serializeDates(data);
      serializeNumbers(data);
      const obj = removeFieldsEmployee(data);

      promises.push(collection.findOneAndUpdate({id_servidor: obj.id_servidor}, {$set: {...obj}})
...
async function loadSalaryData(csvData: Converter, collection: Collection) {
  ...
  return new Promise<void>((resolve, reject) => {
    csvData.subscribe(async (data, index) => {
      ...
      if (index % 40000 == 0) {
        await collection.insertMany(objects, {ordered: false})
...
function removeFieldsSalary(data: Record<string, any>) {
  const keysSalary = {
    ...
    'VERBAS INDENIZATÓRIAS REGISTRADAS EM SISTEMAS DE PESSOAL - CIVIL (R$)(*)': 'verbas_indenizatorias_civil',
    'VERBAS INDENIZATÓRIAS REGISTRADAS EM SISTEMAS DE PESSOAL - MILITAR (R$)(*)': 'verbas_indenizatorias_militar'
  }
